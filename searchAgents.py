...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.initialState = [0,0,0,0] # At the beginning all corners are not visited
    ...
    def isGoalState(self, state):
        ...
        #thus if length of the corner list in state is 4
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            nextState = self.getNextState(state, action)
            nextStateCost = self.getActionCost(state, action, nextState)
            children.append((nextState, action, nextStateCost)) #update the state with the new remaining corners
    ...
    def getNextState(self, state, action):
        ...
        nextx, nexty = int(x + dx), int(y + dy)
        "*** YOUR CODE HERE ***"
        nextPosition = (nextx, nexty) #next Position
        corners = list(state[1])
        if (nextPosition in self.corners) and (nextPosition not in corners):
            corners.append(nextPosition) #if updated position is not in corners, update the state with the new remaining corners
        return nextPosition, tuple(corners)
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if corner not in visitedCorner:  # corner which is not visited
            #count = count + 1
            maxDistance = max(maxDistance, util.manhattanDistance(state[0], corner))
    #if count:
    #    return maxDistance * 1.0 / count
    #else:
    #    return 0
    return maxDistance
...
def foodHeuristic(state, problem):
    ...
    for foodPosition in foodList:
        dis = mazeDistance(position, foodPosition, problem.startingGameState)
        if dis > maxDistance:
            maxDistance = dis
    return maxDistance
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        return (x,y) in self.food.asList() #goal state is reached when it encounters a dot it can eat
        util.raiseNotDefined()
